...
export default function Participacion() {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.tipo) return setError("Selecciona el tipo de participación.");
    if (!form.descripcion || form.descripcion.trim().length < 10)
      return setError("Describe tu interés con al menos 10 caracteres.");
    ...
    try {
      ...
      if (recaptchaReady && window.grecaptcha) {
        try {
          recaptchaToken = await window.grecaptcha.execute(SITE_KEY, { action: "participacion_submit" });
        } catch (e) {
    ...
    <>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto bg-white p-6 rounded shadow">
          <h1 className="text-2xl font-semibold mb-4">Participación</h1>

          <p className="mb-2 text-gray-700">
            Si deseas participar como voluntario o prestador de servicio social, puedes dejarnos tus datos a través de este formulario.
          </p>

          <p className="mb-2 text-gray-700">
            La información enviada será utilizada para establecer contacto y valorar posibles espacios de participación.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">Tipo de participación <span className="text-red-600">*</span></label>
            ...
            <div>
              ...
              <textarea value={form.descripcion} onChange={update("descripcion")} rows={6} className="w-full border rounded px-3 py-2" required placeholder="Describe tu disponibilidad, experiencia, etc." />
            </div>
            ...
            {success ? (
              <div className="bg-green-50 border border-green-200 p-4 rounded">
                <p className="font-medium">Registro recibido</p>
                <p className="mt-2 text-sm text-gray-600">Gracias, nos comunicaremos contigo en breve.</p>
              </div>
            ) : (
